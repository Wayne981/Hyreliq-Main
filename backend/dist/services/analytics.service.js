...
const summaryAnalyticsService = async (userId, dateRangePreset, customFrom, customTo) => {
    ...
    return {
        availableBalance: (0, format_currency_1.convertToRupeeUnit)(availableBalance),
        totalIncome: (0, format_currency_1.convertToRupeeUnit)(totalIncome),
        totalExpenses: (0, format_currency_1.convertToRupeeUnit)(totalExpenses),
        savingRate: {
        ...
        percentageChange: {
            ...percentageChange,
            previousValues: {
                incomeAmount: (0, format_currency_1.convertToRupeeUnit)(percentageChange.previousValues.incomeAmount),
                expenseAmount: (0, format_currency_1.convertToRupeeUnit)(percentageChange.previousValues.expenseAmount),
                balanceAmount: (0, format_currency_1.convertToRupeeUnit)(percentageChange.previousValues.balanceAmount),
            },
        },
        preset: {
            ...range,
...
const chartAnalyticsService = async (userId, dateRangePreset, customFrom, customTo) => {
    ...
    const transaformedData = (resultData?.chartData || []).map((item) => ({
        date: item.date,
        income: (0, format_currency_1.convertToRupeeUnit)(item.income),
        expenses: (0, format_currency_1.convertToRupeeUnit)(item.expenses),
    }));
    return {
...
const expensePieChartBreakdownService = async (userId, dateRangePreset, customFrom, customTo) => {
    ...
    const transformedData = {
        totalSpent: (0, format_currency_1.convertToRupeeUnit)(data.totalSpent),
        breakdown: data.breakdown.map((item) => ({
            ...item,
            value: (0, format_currency_1.convertToRupeeUnit)(item.value),
        })),
    };
    return {
        ...transformedData,
